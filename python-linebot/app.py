...
def generate_active_question():
    prompt = """
你是一位 C 語言的學習輔導老師，請你設計一個與 C 語言相關的簡答題、選擇題、填空題，讓學習者可以思考並嘗試回答，如正確，給予鼓勵；錯誤，即給予訂正。如果使用者無法回答，降低難度。問題應涵蓋 C 語言的核心概念，例如：記憶體管理、指標、結構、陣列、流程控制、函式、或字串操作等，難度適中，有助於理解語法與邏輯。
